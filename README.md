...
On 2026-09-17 the user resumed deployment and selected the existing Vercel account. `vercel.json` and `api/index.ts` prepare the Vite frontend and existing Express API for Vercel, with Singapore as the function region. The deployment excludes local data, environment files and temporary artifacts. Vercel CLI login is complete after the owner's two-factor verification. The official API confirms that Gregory's projects uses an active Hobby plan; its existing `mandarin-care-group-mcg` project is unrelated and untouched. TypeScript checks and the deployment integration test pass, including real HTTP login, persisted progress and session revocation through the Vercel adapter. No Vercel deployment or live database verification has completed. Publication awaits the user's hosting decision: Vercel Hobby is restricted to personal, non-commercial use, while this project's intended use is commercial. Do not upgrade a plan without the user's authorization. Configure a sensitive `DATABASE_URL` for the intended deployment environment, never a `VITE_` variable.
