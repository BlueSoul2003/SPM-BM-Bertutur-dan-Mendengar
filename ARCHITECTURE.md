...
## Vercel educational pilot — 2026-09-17

The user clarified that the current release is educational and non-commercial, selected the existing Vercel Hobby account, and explicitly authorized saving the Bual SPM role credential in project `bual-spm` as a Production-only sensitive `DATABASE_URL`. The prior credential-transfer approval block is resolved. No plan upgrade, payment service or paid add-on was enabled. The unrelated existing project is untouched.

The public pilot is https://bual-spm.vercel.app. React/Vite assets are on the CDN; `api/index.ts` adapts the existing Express API to a Singapore Node function, reuses the pool, and retries failed initialization. The adapter trusts the single Vercel proxy for client IP rate limits. Supabase private schema `bual` and the dedicated CRUD-only role remain unchanged.

Cloud verification exposed and fixed three deployment issues: the data exclusion must be `/data/` to retain `src/data`; shared data re-exports need `.js` suffixes in Node ESM; Supabase's shared pooler needs its official public CA. The bundled certificate is from the dashboard's HTTPS download link, and the connection enforces `verify-full` (CA and hostname checks). Vercel explicitly includes the certificate file. No TLS verification was disabled.

TypeScript, frontend build and deployment HTTP integration tests pass. The live public API passed synthetic registration, login, check-in, listening scoring, duplicate-attempt prevention, retained progress after re-login, disabled-AI quota preservation, logout and revoked-token rejection. Verified managed database connectivity uses the private schema. Sequential smoke requests took about 50–600 ms; concurrency capacity and cold-start distributions are not established.

Manual CLI deployments are in use; the GitHub integration is not connected. Keep student-direct mode enabled and SMTP/payment/AI credentials absent. This is an educational pilot: provider eligibility and assessment quality, real phone audio/microphone checks, recovery email delivery, restore drills and load testing remain before a broader release. Reassess hosting before future commercial use.
