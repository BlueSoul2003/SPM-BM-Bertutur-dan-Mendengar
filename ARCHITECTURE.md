...
The user explicitly approved public hosting with account/progress storage after an automatic approval review initially required clarification. A new Netlify project `bual-spm-preview` was created. The deploy then failed at Netlify Database provisioning because that feature is unavailable for this account (HTTP 403). The backend now supports an external PostgreSQL connection, and `scripts/deploy-schema.sql` holds the initial schema. An external database choice is pending; no successful live deployment or database verification is claimed. Local deployment-adapter tests exercise registration, authorization, check-in persistence and logout against the deploy SQL schema.
